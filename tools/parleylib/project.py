...
Standalone entry point: tools/project.py (a shim) or `parley project`.
"""
...
def visible_to(r: V.Rec, pid: str, chairs) -> bool:
    return pid in chairs or r.visibility == STAR or pid in r.visibility
...
class Projector:
    def __init__(self, records: list, registry: dict, pid: str, git: Git, show_ts: bool):
        ...
        self.chairs = {p["id"] for p in registry["participants"] if p.get("role") == "chair"}
        self.chair = sorted(self.chairs)[0] if self.chairs else None
        self.is_chair = pid in self.chairs
        self.git, self.show_ts = git, show_ts
        self.all = records
        self.visible = [r for r in records if visible_to(r, pid, self.chairs)]
    ...
    def thread_line(self, root: str) -> str:
        ...
        # replies still owed do not hold a thread back from ready (PROTOCOL §4.1); the chair sees them anyway
        replies = [o for o in self.rp.obligations.values() if o.thread == root and o.kind == "answer"]
        if replies:
            s += " — replies owed: " + "; ".join(f"{o.party} re #{o.rec}" for o in replies)
        return s
    ...
    def standing_agent(self, delta: list, last_id: str | None, dropped: list) -> str:
        ...
        if not self.chairs and self.rp.chair_queue:
            out.append("no chair in this parley: ready threads stay open — " + ids(sorted(self.rp.chair_queue)))
        if dropped:
    ...
    def standing_chair(self, delta: list, last_id: str | None) -> str:
        rp = self.rp
        agent_obs = [o for o in rp.obligations.values() if o.party not in self.chairs]
        live_noms = {p: sorted(set(v)) for p, v in rp.nominations.items() if v and p not in self.chairs}
        if not agent_obs and not live_noms:
            ...
            floor = "; ".join(bits)
        others = sorted(self.chairs - {self.pid})
        who = f"you are {self.pid}, chair" + (f" (with {', '.join(others)})" if others else "")
        out = [f"--- standing: {who} · log through #{last_id or 'none'} · floor: {floor} ---"]
        if rp.chair_queue:
            out.append("decide:")
        ...
        asks = [o for o in rp.obligations.values() if o.party == self.pid]
        if asks:
